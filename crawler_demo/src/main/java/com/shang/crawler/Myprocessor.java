...
//        System.out.println(page.getHtml().xpath("//*[@id=\"nav\"]/div/div/ul/li[13]/a/text()").toString());
        //目标地址正则匹配
...
//        System.out.println(page.getHtml().xpath("//*[@id=\"mainBox\"]/main/div[1]/div/div/div[1]/h1/text()").toString());
        ...
        //ConsolePipeline 控制台输出  //*[@id="nav"]/div/div/ul/li[5]/a
        page.addTargetRequests(page.getHtml().links().regex("https://blog.csdn.net/[a-z 0-9 -]+/article/details/[0-9]{8}").all());
...
//                .addPipeline(new FilePipeline("/Users/shangjiapeng/desktop/crawler/file"))  // FilePipeline 文件保存
                .addPipeline(new JsonFilePipeline("/Users/shangjiapeng/desktop/crawler/json"))  // 以json方式保存
//                .addPipeline(new MyPipeline())  //定制化输出
                //使用setScheduler来设置Scheduler----内存队列QueueScheduler()
//                .setScheduler(new QueueScheduler())
                //设置文件队列使用文件保存抓取URL，可以在关闭程序并下次启动时，从之前抓取到的URL继续抓取
//                .setScheduler(new FileCacheQueueScheduler("/Users/shangjiapeng/desktop/crawler/scheduler"))
